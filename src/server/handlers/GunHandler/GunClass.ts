...
export class Gun {
	...
	private Activated(d: Vector3, o: Vector3) {
		...
		if (this.GetAmmo()! - 1 >= 0 && GetPlayerState(this.player.Name)?.canUseGun) {
			...
			UpdateGunState(this.player.Name, (s) => ({
				...s,
				lastTimeUsed: DateTime.now().UnixTimestampMillis,
			}));
			this.Fire(d, o);

			if (this.gun.FindFirstChild("Handle")!.FindFirstChild("Fire")) {
				(this.gun.FindFirstChild("Handle")!.FindFirstChild("Fire") as Sound).Play();
			}
		}
		...
		task.spawn(() => {
			while (this.debounce) {
				if (
					DateTime.now().UnixTimestampMillis - GetGunState(this.player.Name)?.lastTimeUsed! >
					this.gunConfig.delay
				)
					this.debounce = false;
				task.wait();
			}
	...
	private ConnectEvents() {
		this.trash.add(
			messaging.server.on(Message.Input, (player, keycode) => {
		...
		this.trash.add(
			messaging.server.on(Message.Fire, (p, { DirectionVector, Origin }) => {
				if (this.player === p) this.Activated(DirectionVector, Origin);
			}),
		);
	}
